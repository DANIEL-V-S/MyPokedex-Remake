...
public class RcvTypesAdapter extends RecyclerView.Adapter<RcvTypesAdapter.ViewHolder> {
    ...
    public RcvTypesAdapter(Context context, Cursor cursor) {
        setContext(context);
        setCursor(cursor);
    ...
    @NonNull
    @Override
    public RcvTypesAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new RcvTypesAdapter.ViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.list_single_line, parent, false));
    }

    @Override
    public void onBindViewHolder(@NonNull RcvTypesAdapter.ViewHolder holder, int position) {
        ...
        holder.getConstraintLayout().setOnClickListener(v -> {
            ...
            builder.setItems(new String[]{context.getString(R.string.dialog_what_to_do_view_details), context.getString(R.string.dialog_what_to_do_update), context.getString(R.string.dialog_what_to_do_delete)}, (dialog, which) -> {
                switch (which) {
                    ...
                    break;
                    case 1: {
                        getJm().updateTypeDialog(getContext(), holder.getConstraintLayout(), getDataBase(), new String[]{holder.getTxtCodigo().getText().toString()}, this, holder.getAdapterPosition());
                    }
                    break;
                    case 2: {
                        getJm().deleteTypeDialog(getContext(), holder.getConstraintLayout(), getDataBase(), new String[]{holder.getTxtCodigo().getText().toString()}, this, holder.getAdapterPosition());
                    }
                    break;
                    default: {
                    }
